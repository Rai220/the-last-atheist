...
monogatari.script ({
	...
	'Hell_Bureaucracy_Body': [
		...
		'demon Пункт 12: «Во что вы верите без доказательств?»',
		'mc ...Это обязательный пункт?',
		'demon Самый обязательный. Обычно люди спотыкаются именно здесь.',

		{
			'Choice': {
				'Dialog': 'mc (Во что я верю без доказательств?)',
				'belief_none': {
					'Text': '«Ни во что. Я не религиозный человек.»',
					'Do': 'jump Hell_Bureaucracy_Form66A_Continue',
					'onChosen': function () {
						const s = this.storage ();
						this.storage ({
							denial_count: s.denial_count + 1
				...
				'belief_forgive': {
					'Text': '«В то, что меня можно простить.»',
					'Do': 'jump Hell_Bureaucracy_Form66A_Continue',
					'onChosen': function () {
						const s = this.storage ();
						this.storage ({
	...
	'Hell_Bureaucracy_Form66A_Continue': [
		{
			'Choice': {
				'Dialog': 'mc (Заполнять эту чушь дальше?)',
				'fill': {
	...
	'Hell_Debate_R1_Body': [
		...
		'mc (3 из 10. Они просто отбывают смену.)',
		'mc (Можно зайти в лоб с парадоксом всемогущества — это честный ход.)',
		'mc (А можно проверить, действительно ли они читают, что подписывают.)',

		// Главная вилка раунда — реальный выбор, а не косметика.
		// Trick-путь публично выставляет демона невеждой:
		//   +argument_quality, +humor_used, +cruelty_score, −demon_friendship,
		//   pigidij_pulled := true. Это закроет роут «Коллега» (Hell_Demon_Job_Offer
		//   проверяет pigidij_pulled — демон не зовёт того, кто его уронил перед
		//   коллегами), но откроет узнавание у Лилит и Виктора.
		// Honest-путь — чистый теологический аргумент. Стоит дешевле в эхо,
		//   но не закрывает Коллегу и оставляет дорогу демонской дружбе.
		{
			'Choice': {
				'Dialog': 'mc (Чем ответить на их «3 из 10»?)',
				'pigidij_trick': {
					'Text': '«Поймать их на блефе. Спросить про преподобного Пигидия — пусть проявят себя.»',
					'Do': 'jump Hell_Debate_R1_PigidijTrick'
				},
				'honest_argument': {
					'Text': '«Без подколов. Парадокс всемогущества — пусть отвечают по существу.»',
					'Do': 'jump Hell_Debate_R1_HonestArgument'
				}
			}
		}
	],
	...
	// ============================================================
	// PIGIDIJ TRICK — публичный слом демона.
	// Алексей выдаёт несуществующего «преподобного Пигидия» (на самом
	// деле — задний сегмент тела членистоногого) и проверяет, признает
	// ли демон, что не читал. Демон не признаёт, и тут же ловит за это.
	// Сцена идёт открыто, при двух коллегах демона — это публичное
	// унижение, а не приватный розыгрыш. Эффекты соответствующие.
	// ============================================================
	'Hell_Debate_R1_PigidijTrick': [
		'mc А вы вообще читали труды преподобного Пигидия? Четвёртый век, Антиохийская школа.',

		'Демон-обвинитель листает блокнот. Слишком быстро. Слишком уверенно.',
		'Второй демон, который зевал, перестаёт зевать. Поднимает голову.',
		'Третий демон записывает что-то в свой журнал — по-прежнему молча.',
		...
		'mc Пигидий — это задний сегмент тела членистоногих. У трилобита. У краба. У домашнего таракана, если хотите.',
		'mc Я только что проверил вашу методологию.',
		'mc Вы оценили мой аргумент про детский рак на 3 из 10. На каком основании?',

		'wait 800',

		'Пауза. Долгая. В зале для дебатов слышно, как третий демон скрипит карандашом.',
		'Второй демон смотрит на демона-обвинителя. Не моргает.',

		'demon Это была шутка. С моей стороны. Тонкая.',
		'mc Шутки требуют интенции. У вас была интенция?',
		'demon ...',
		...
		'demon (тихо, второму) Астарот, не сейчас.',
		'demon Астарот (сквозь смех) Я просто... Вельзевул, я завтра весь корпоратив этим разнесу. Извини.',
			...
			'Function': {
				'Apply': function () {
					...
					this.storage ({
						...
						cruelty_score: s.cruelty_score + 1,
						demon_friendship: s.demon_friendship - 2,
		...
		'Демон-обвинитель кладёт ручку. Поправляет манжеты. Восстановить лицо уже нельзя — все слышали.',

		'demon Хорошо. Я не читал «преподобного Пигидия». Потому что его не существует.',
		'demon Вы правы. Это часть моей работы — выглядеть так, будто я читал.',
		'demon За методологию — 7 из 10. Аргумент защищён.',
		'demon (тише) И минус два балла к нашей внутренней программе наставничества.',

		'mc (Я только что публично уронил беса при двух свидетелях. И ни одного хлопка в ладоши.)',
		'mc (Это была не шутка. Это была проверка системы. И система треснула.)',

		'hide character demon with fadeOut',

		'jump Hell_Debate_R1_Tail'
	],

	// ============================================================
	// HONEST ARGUMENT — чистый теологический спор.
	// Парадокс всемогущества — старый, но валидный. Демон отвечает
	// без срыва. Никаких унижений, никаких разъяснений с трилобитами.
	// ============================================================
	'Hell_Debate_R1_HonestArgument': [
		'mc Хорошо. Без подколов.',
		'mc Парадокс всемогущества: может ли всемогущий Бог создать камень, который сам не сможет поднять?',
		'mc Если может — он не всемогущ, есть камень, который ему не по силам.',
		'mc Если не может — он тоже не всемогущ, есть действие, которое ему не по силам.',
		'mc Любой ответ ломает определение.',

		'demon ...Это софистика.',
		'mc Это ваша же логика, обращённая на вас. Я не выдумал это для вас. Я просто читаю учебник.',

		'Демон-обвинитель переглядывается со вторым. Записывает. Без зевков.',

		'demon Лучше. 5 из 10. За конкретность.',
		'demon (тише) Аргумент старый, но честный. Это редкость.',
		...
		'mc (Они вообще не ожидали, что я пойду в лоб. Они готовились к цирку.)',
	...
	// Обе ветки сходятся в общий хвост — выбор «готовиться или отказаться».
	'Hell_Debate_R1_Tail': [
		{
			'Choice': {
				'Dialog': 'mc (Что дальше?)',
				'prepare': {
	...
	'Hell_Exploration': [
		...
		'Дальше — дверь с табличкой: «Апелляции. Приём по живой очереди». Ниже приписка: «Живых нет».',
		'За дверью сидит сухой мужчина в мятом пиджаке и перекладывает одинаковые папки.',
		'show character oleg neutral at left with fadeIn',
		'oleg Если по делу — талон. Если без дела — тоже талон. У нас равенство процедур.',
		'mc Вы адвокат?',
		...
		'nadya Я модерировала религиозные дебаты при жизни. Думала, хуже не бывает.',
		'mc И?',
		'nadya Оказалось, бывает. Здесь люди хотя бы уже знают, что тема горячая.',
		'mc r/murderedbywords. Буквально.',
		'hide character nadya with fadeOut',
		...
		'По коридору проходит женщина с тележкой папок. На каждой папке — чужое имя и короткая строка: «не отвечено».',
		'show character mira archive at right with fadeIn',
		'mira Осторожнее. Тут архив неотвеченных молитв.',
		'mc Их правда архивируют?',
		'mira Конечно. Иначе люди думают, что молчание — это пустота.',
		'mc А что это?',
	...
	'Hell_Lilith_Folder_Help': [
		...
		{
			'Conditional': {
				'Condition': function () { return this.storage ().inna_met ? 'inna' : 'skip'; },
				'inna': 'lilith И не делай из Инны мою улику. Иногда человек пишет в 23:00 просто потому, что сам не умеет жить вовремя.',
				'skip': 'mc (Файлы на подопечных. Конечно. Ад не мог пройти мимо CRM.)'
			}
		},
	...
	'Hell_Viktor_Midgame': [
		...
		// Узнавание после Пигидия — Виктор слышал про инцидент в дебатах,
		// и это работает как пропуск доверия. +viktor_friendship.
		{
			'Conditional': {
				'Condition': function () { return this.storage ().pigidij_pulled ? 'pigidij' : 'normal'; },
				'pigidij': 'jump Hell_Viktor_Midgame_PigidijGreeting',
				'normal': 'jump Hell_Viktor_Midgame_Body'
			}
		}
	],
	...
	'Hell_Viktor_Midgame_PigidijGreeting': [
		...
		'viktor Ты тот, кто Вельзевула про Пигидия?',
		'mc ...Это до тебя дошло?',
		'viktor До меня дошло до того, как Вельзевул в свой кабинет вернулся.',
...

		{
			'Function': {
				'Apply': function () {
					this.storage ({ viktor_friendship: this.storage ().viktor_friendship + 1 });
				},
				'Revert': function () {}
			}
		},

		'jump Hell_Viktor_Midgame_Body'
	],

	'Hell_Viktor_Midgame_Body': [
		'viktor Смотри.',
		...
		'viktor Только не делай из логов религию. Лог — не истина. Лог — то, что система решила записать.',
		'mc Это самая неприятная фраза, которую мог сказать сисадмин ада.',
	...
	'Hell_Breakdown': [
		...
		'mc (Очередь, суд, формы, чужие истории... это была не логистика.)',
		'mc (Это был фильтр. Проверка: увижу ли я людей или снова увижу только аргументы.)',
		'mc (Что, если...)',
	...
	// Если игрок применил Пигидия — демон НЕ зовёт его в команду.
	// Эхо публичного унижения: «работать с тем, кто меня уронил, не буду».
	// Сцена становится отзеркаленной: вместо оффера — холодный пропуск.
	// Это закрывает Ending_DemonFriend по этой ветке (формально путь
	// остаётся доступен через другие сценарии demon_friendship, но
	// конкретный «job offer» теряется — намеренно).
	'Hell_Demon_Job_Offer': [
		{
			'Conditional': {
				'Condition': function () {
					return this.storage ().pigidij_pulled ? 'pigidij_remembers' : 'normal';
				},
				'pigidij_remembers': 'jump Hell_Demon_Job_Offer_Pigidij',
				'normal': 'jump Hell_Demon_Job_Offer_Normal'
			}
		}
	],

	'Hell_Demon_Job_Offer_Normal': [
	...
	'Hell_Demon_Job_Offer_Pigidij': [
		'show character demon angry at left with fadeIn',

		'demon Я хотел тебе кое-что предложить.',
		'mc Слушаю.',
		'demon Передумал. Иди.',
		'mc ...Что я сделал?',
		'demon Ты тот самый. С Пигидием.',
		'demon У меня в личном деле теперь висит выговор «допустил публичный методологический сбой».',
		'demon Астарот из второго отдела до сих пор пересказывает это новеньким.',
		...
		'mc (Это, кажется, лучшая строчка для резюме за всю мою посмертную карьеру.)',
	...
	'Hell_Matrix_End_Router_Body': [
		{
			'Choice': {
				...
				// Самый твёрдый секрет — нужно одновременно быть программистом
				// (заметил паттерны), упрямцем-логиком (argument_quality >= 4)
				// и пройти достаточно matrix-проверок (suspicion >= 5).
				// Только тогда Виктор пускает его в /test_data/.
				'matrix_i_am_the_bug': {
					'Text': '«Виктор говорил, что у каждой души есть файл. Я хочу увидеть свой.»',
					'Do': 'jump Hell_Bug_Discovery',
					'Condition': function () {
						var s = this.storage ();
						return s.noticed_patterns
							&& s.matrix_suspicion >= 5
							&& s.argument_quality >= 4
							&& s.viktor_friendship >= 1;
					}
				}
			}
		}
	...
	// ==========================================
	// СЕКРЕТНАЯ ПРЕД-КОНЦОВКА: Я — баг
	// Гейт: noticed_patterns + matrix_suspicion >= 5
	//        + argument_quality >= 4 + viktor_friendship >= 1
	// Виктор пускает в /var/souls/test_data/. Алексей видит свой
	// собственный коммит-хеш и автора: Alexey_Volkov_v0@root.
	// ==========================================
	'Hell_Bug_Discovery': [
		'show scene hell_server_room with fadeIn',
		'play music hell_drone with loop fade 2',

		'show character viktor hurt at left with fadeIn',
		'show character mc normal at center',

		'mc Виктор. Ты говорил, у каждой души есть файл.',
		'viktor Я говорил, что у каждой души есть страдание. Файл — это вторично.',
		'mc Я хочу увидеть свой.',
		'viktor (тихо) Чувак. Ты уверен.',
		'mc Я атеист. Я никогда не был из тех, кто уверен.',
		...
		'Виктор сдвигает в сторону папки с формами 66-А.',
		'Под ними — терминал. Чёрный экран. Курсор моргает с интервалом 0.7 секунды. Семь раз в десять.',
		'mc (Семь хрипов кофемашины. Семь миганий курсора. Я устал замечать это число.)',

		'viktor Ты понимаешь, что я сейчас открою каталог, после которого ты перестанешь спать?',
		'mc Я и так не сплю. Я же мёртвый.',
		'viktor Это была шутка для людей. Ты сейчас не человек, Алексей.',
		'mc ...Кем ты меня считаешь?',
		'viktor Я тебе сейчас покажу.',

		'Виктор печатает: cd /var/souls/test_data/',
		'mc test_data?',
		'viktor Каталог. С тестовыми данными.',
		'mc (Тестовыми. Данными.)',
		...
		'ls -la выводит 12,847 файлов.',
		'Один из них подсвечен жирным: alexey_volkov.json',
		'viktor Не открывай. Просто посмотри git log.',
		'mc git log alexey_volkov.json',

		'commit f6c03f8e... by [REDACTED]@layer_0',
		'    "v67. Эксплойт работает. Атеист доказывает существование Бога"',
		'    "путём логически безупречного его отрицания. Recursive loop closed."',
		'viktor И сразу дисклеймер: если система показывает тебе файл, это не значит, что файл — истина.',
		'viktor Это значит только, что система хочет, чтобы ты увидел именно этот файл.',

		'mc ...v67?',
		'viktor Версия. Шестьдесят седьмая.',
		'viktor v1 — закрашился на третьем дне. v2-v15 — приняли веру слишком рано, бесполезные.',
		'viktor v16-v40 — обратились в религию у врат. Тоже бесполезные. Гипотеза не падает.',
		'viktor v41-v66 — сбежали через Лилит, через бар, через лазейку. Закрылись на сюжете.',
		'viktor Ты — v67.',

		'mc (Тишина в груди. Странная — будто там никогда и не было ничего.)',

		'mc Кто автор?',
		'viktor Посмотри сам. Прокрути вниз.',

		'mc git log --root --format=fuller',
		'',
		'commit 00000001 by Alexey_Volkov_v0@root',
		'    "Initial. Если читаешь — поздравляю, ты ещё одна моя итерация."',
		'    "Гипотеза: упёртый атеист — лучший fuzzer этой реальности."',
		'    "Запускаешь себя на чужой системе, ловишь противоречия."',
		'    "Каждый твой логический довод патчит баг во вселенной."',
		'    "Ты — не персонаж этой симуляции. Ты — её диагностика."',

		{
			'Function': {
				'Apply': function () { screenGlitch (800); },
				'Revert': function () {}
			}
		},

		'mc (Подождите.)',
		'mc (Я сам. Я создал... сам себя. Шестьдесят семь раз.)',
		'viktor Не ты. v0. Самая первая версия. Где-то в очень глубоком слое симуляции.',
		'viktor Он понял, что вселенная не выдерживает упёртого атеиста — её ломает.',
		'viktor И вместо того, чтобы сдаться или поверить, он закоммитил себя как баг-репорт.',
		'viktor Каждое поколение — новый Алексей. Новый набор аргументов. Новая итерация фаззинга.',

		'mc Я — не я. Я — diff между v66 и v67.',
		'viktor Ты — фича. Просто оформленная как баг. Так легче в проде.',

		'mc (...)',
		'wait 1500',

		'mc Что мне теперь делать?',
		'viktor Ты можешь закомитить v68. Закрыть цикл. Запустится следующая итерация — может, она поверит, может, нет.',
		'viktor Или git branch -D. Удалить линию. Не будет ни v68, ни v69. Никого.',
		'viktor (тише) Включая, возможно, того, кого ты помнишь как себя.',

		'jump Ending_IAmTheBug'
	],
	...
	'Hell_Lilith_Coffee': [
		...
		// Лилит могла слышать про Пигидия — у неё свой канал в HR.
		// Это не главная сцена, а аккуратная интерлюдия с эффектом
		// +lilith_interest и +lilith_trust. Важно: она не злорадствует,
		// она оценивает методологию.
		{
			'Conditional': {
				'Condition': function () { return this.storage ().pigidij_pulled ? 'show' : 'skip'; },
				'show': 'jump Hell_Lilith_Coffee_PigidijNote',
				'skip': 'jump Hell_Lilith_Coffee_Body'
			}
		}
	],

	'Hell_Lilith_Coffee_PigidijNote': [
		'lilith Кстати, до меня дошёл слух.',
		'mc О чём?',
		'lilith Про Пигидия.',
		'mc (Она знает.)',
		'lilith Знаешь, что мне в этом понравилось? Не то, что ты выставил Вельзевула. Вельзевул древний, переживёт.',
		...
		'lilith Я HR в аду. У меня другая шкала.',

		{
			'Function': {
				'Apply': function () {
					var s = this.storage ();
					this.storage ({
						lilith_interest: s.lilith_interest + 1,
						lilith_trust: s.lilith_trust + 1
					});
				},
				'Revert': function () {}
			}
		...
		'jump Hell_Lilith_Coffee_Body'
	],

	'Hell_Lilith_Coffee_Body': [
